...
export default function Page() {
  ...
  const getImages = async (path) => {
    ...
    const res = await fetch(
      `/api/images?token=${token}&owner=${owner}&repo=${repo}&branch=${branch}&path=${
        path || ""
      }`
    );
      ...
      <div className="bg-white border shadow rounded px-3 py-2 flex-row md:flex items-center">
        <div className="flex items-center">
          ...
          <input
            ...
            onBlur={handleParseGithub}
            type="text"
            className="border text-gray-600 rounded h-7 px-1 ml-2 focus:outline-none"
          />
        </div>
        ...
        <div className="flex items-center md:ml-2">
          ...
          <input
            ...
            type="text"
            className="w-80  border text-gray-600 rounded h-7 px-1 ml-2 focus:outline-none"
          />
        </div>
        <div className="flex items-center md:ml-2">
        ...
        <div className="flex items-baseline ml-auto">
          ...
          <Link href="/help" className="text-gray-500 text-sm ml-2 underline">
            *帮助
          </Link>

          <button
            className={`${
              hasSetting ? "bg-green-300" : "bg-green-400"
            } text-white text-sm border rounded px-3 py-1 ml-2`}
      ...
      return (
        <>
          <p className="text-gray-500 text-sm ml-3">
            免费用户仅显示今日上传记录，付费用户支持目录树和删除功能
          </p>
          <Link
            href="/help#update"
            className="text-gray-600 text-sm ml-3 underline cursor-pointer"
          >
            *去升级
          </Link>
        </>
  ...
  const renderCopyButton = (title, item) => {
    return (
      <div
        className="bg-green-400 text-white text-sm border rounded px-3 py-1 cursor-pointer flex items-center"
        onClick={() => handleCopy(title, item)}
      >
        <svg
          t="1691750350692"
      ...
      <div className="flex-row md:flex mt-3 items-start">
        <div className="bg-white border shadow rounded md:w-1/3">
          ...
          <div
            ...
            className="flex-row py-10 cursor-pointer hover:bg-cyan-100"
          >
            <Image
          ...
          <div className="py-2">
            {list.map((item, index) => {
              return (
                <div key={index}>
                  <div
                    className="flex-row md:flex items-center px-2 mt-1 cursor-pointer"
                    onClick={() => vip && getImages(item.path)}
                  >
                    <Image
                      src={
                        item.fold
                          ? "/assets/folder-open.png"
                          : "/assets/folder.png"
                      }
                      alt="image"
                      width="32"
                    ...
                    />
                    <p className="text-gray-500 text-sm ml-0 md:ml-3">
                      {item.path}
                    </p>
                  </div>
                  ...
                  {item.children.map((child, idx) => {
                    return (
                      <div
                        key={idx}
                        className="pl-4 flex-row md:flex items-center px-2 mt-2"
                      >
                        <Image
                          src={`https://wsrv.nl/?w=40&h=40&url=${child.raw_url}`}
                          alt="image"
                          width="32"
                          height="32"
                          className="hidden md:block"
                        ...
                        <div className="ml-auto flex items-center">
                          {vip && (
                            <div
                              className="bg-red-300 text-white text-sm border rounded px-3 py-1 cursor-pointer flex items-center"
                              onClick={() =>
                                handleDel(item.path, child.path, child.sha)
                              }
                            >
                              <svg
                                t="1691859359859"
